...
public class PrototypeGenerator : MonoBehaviour
{
    ...
    public string path = "Assets/Prefabs/MapGeneration/A";
    ...
    [ContextMenu("Generate Prototypes - Context Menu")]
    ...
    public void UpdatePrototypes()
    {
        ...
        for (int i = 0; i < protoypePrefabs.Count; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                Debug.Log(protoypePrefabs[i].prefab.name);
    ...
    public static Prototype CreateMyAsset(string assetFolder, string name, string rotationIteration)
    {
        Prototype asset = ScriptableObject.CreateInstance<Prototype>();
#if UNITY_EDITOR
        AssetDatabase.CreateAsset(asset, assetFolder + "/" + name + "_" + rotationIteration + ".asset");
    ...
    public void DisplayPrototypes() // could be deleted if not needed in game (shows available prototypes)
    {
